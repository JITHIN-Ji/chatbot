...
class VectorStoreManager:
    def __init__(self, user_id: str, index_path: str = settings.VECTOR_STORE_PATH, dimension: Optional[int] = None):
        ...
        self.index_path_base = index_path # Store base path before .index or .meta.json
        self.index_file_path = self.index_path_base + ".index"
        self.meta_file_path = self.index_path_base + ".meta.json"
        # self.doc_ids_file_path = self.index_path_base + ".doc_ids.txt" # Consolidating into meta.json

        self.index_dir = os.path.dirname(self.index_path_base)
        os.makedirs(self.index_dir, exist_ok=True)
        
        self.index: Optional[faiss.Index] = None
        # self.doc_ids: List[str] = [] # Now part of metadata entries
        self.metadata_list: List[Dict[str, Any]] = [] # Stores metadata, each entry corresponds to a vector
                                                 # Example: {'doc_id': 'xyz', 'chunk_text': '...', 'chunk_index': 0}
        ...
        if os.path.exists(self.index_file_path) and os.path.exists(self.meta_file_path):
            ...
            self._save_metadata_list() # Save empty metadata initially
        else:
            logger.warning(f"Vector store index {self.index_file_path} not found and dimension not provided for initialization.")
            # Index remains None until add_embeddings is called with the first set of embeddings
    ...
    def add_embeddings(self, embeddings: np.ndarray, metadata_entries: List[Dict[str, Any]]):
        ...
        if self.index is None: # Still None if initialization failed
        ...
        if embeddings.shape[1] != self.index.d:
            ...
            # Consider raising ValueError for critical errors
    ...
    def search(self, query_embedding: np.ndarray, k: int = 5, document_id_filters: Optional[List[str]] = None) -> List[Tuple[str, Dict[str, Any], float]]:
        ...
        # If filtering, retrieve more results initially to account for post-filtering
    ...
    def load_index(self):
        try:
            ...
            self.index = None # Reset if loading fails
            self.metadata_list = []
